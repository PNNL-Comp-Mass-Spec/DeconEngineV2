...
namespace DeconToolsV2
{
    ...
    public class clsProcRunner
    {
            ...
            clsRawDataPreprocessOptions fticrPreprocessParameters, bool savePeaks, bool transform)
        {
            ...
            //Engine.ResultChecker.LCMSCheckResults lcmsChecker = null;
            ...
            try
            {
                    ...
                    scanNum = rawData.GetNextScanNum(scanNum))
                {
                    ...
                    // Disable timing (MEM 2013)
                    // rawDataReadTime += (DateTime.Now - currentTime);
                    ...
                    // Disable timing (MEM 2013)
                    // averageTime += DateTime.Now - currentTime;
                    // currentTime = DateTime.Now;
                    ...
                    // Disable timing (MEM 2013)
                    // ticTime += DateTime.Now - currentTime;
                    ...
                    if (HornTransformParameters.UseMZRange)
                    {
                        ...
                        // Disable timing (MEM 2013)
                        // peakDiscoverTime += DateTime.Now - currentTime;
                    }
                    else
                    {
                        // Disable timing (MEM 2013)
                    ...
                    if (savePeaks)
                    {
                        ...
                        // Disable timing (MEM 2013)
                        // peakSaveTime += DateTime.Now - currentTime;
                    }
                    // Disable timing (MEM 2013)
                    // preprocessingTime += (DateTime.Now - currentTime);

                    var numDeisotoped = 0;

                    // Disable timing (MEM 2013)
                    // preprocessingTime += DateTime.Now - startTime;

                    /*
                    if (fileType != FileType.PNNL_UIMF && scanNum % 20 == 0)
                    {
                        // Disable timing (MEM 2013)
                        // DateTime current_t = DateTime.Now;
                        // int all = current_t - startTime;
                        Console.WriteLine(string.Concat(
                            " Raw Reading Time = ", Convert.ToString(rawDataReadTime),
                            " Average Time = ", Convert.ToString(averageTime),
                            " TIC Time = ", Convert.ToString(ticTime),
                            " Peak Discover Time = ", Convert.ToString(peakDiscoverTime),
                            " Peak Save Time = ", Convert.ToString(peakSaveTime),
                            " PreProcessing Time = ", Convert.ToString(preprocessingTime),
                            " preprocess-read= ", Convert.ToString(preprocessingTime-rawDataReadTime)
                            ));
                    }
                    */
                    ...
                    if (transform)
                    {
                        ...
                        while (found)
                        {
                            ...
                            try
                            {
                                ...
                                // Disable timing (MEM 2013)
                                // transformTime += (DateTime.Now - currentTime);
                            ...
                            //if (fileType != DeconToolsV2.Readers.PNNL_UIMF && scan_num % 20 == 0)
                        {
                            ...
                            //int all = (int) (current - startTime).Ticks;
